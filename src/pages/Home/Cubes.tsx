...
const render = (time) => {
  ...
  for (let b = 0, bl = boxes1d.length; b < bl; b++) {
    ...
    const distanceZ =
      1 -
      ((rows * (size.depth + gap) - box.posZ) / (rows * (size.depth + gap)) -
        1);
    box.material.uniforms.distanceZ.value = distanceZ;

    const distanceX =
      1 - Math.abs(box.position.x) / ((cols * (size.depth + gap)) / 3);
    ...
    box.material.uniforms.pulse.value -=
      (box.material.uniforms.pulse.value * 0.1) / (currentSpeed + 1);
  }
...
const MyThreeJSComponent = () => {
  ...
  return (
    <div style={{ width: "100vw", height: "inherit" }} ref={containerRef} />
  );
};
